...
=pssh= 是一个 python 编写可以在多台服务器上执行命令的工具，同时支持拷贝文件，是同类工具中很出色的，类似 =pdsh= 。为方便操作，使用前请在各个服务器上配置好密钥认证访问。项目地址: [[https://code.google.com/p/parallel-ssh][parallel-ssh]]
...
- [[http://linux.die.net/man/1/pssh][pssh]]
- [[http://www.theether.org/pssh/docs/0.2.3/pssh-HOWTO.html][pssh-howto]]

** pdsh

=pdsh= (Parallel Distributed Shell) 可并行的执行对目标主机的操作，对于批量执行命令和分发任务有很大的帮助，在使用前需要配置 ssh 无密码登录，[[http://sourceforge.net/projects/pdsh/][点击下载]]
...
查看哪些主机时间不一样，主机一多，可读性不强，以下使用 =dshbak= 之后可读性变得好了很多
...
- [[https://code.google.com/p/pdsh/wiki/UsingPDSH][Using PDSH]]
