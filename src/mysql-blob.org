...
- [[http://blog.sina.com.cn/s/blog_8e9cceee0101k65j.html][Mysql的大字段问题]]
- [[http://stackoverflow.com/questions/15585602/change-limit-for-mysql-row-size-too-large][Change limit for "Mysql Row size too large"]]
...
1. msyql> stop slave;
2. 修改 slave ~innodb_file_format=BARRACUDA~ 「并添加该值到 my.cnf」和表 ~row_format=dynamic~
...
问题虽然解决了，但是有时候需要追根溯源，特别对于数据安全性相关的，原理性的东西弄明白就会好解释一些问题了。13 号继续跟踪这个问题，在网上找到了这篇文章 [[http://hidba.org/?p=551][innodb使用大字段text，blob的一些优化建议]]，看了一遍，继续查看了相应的资料之后彻底弄明白了。

*** Antelope 和 Barracuda 文件格式

[[https://opskumu.oss-cn-beijing.aliyuncs.com/images/mysql-innodb-file-format.png]]
...
[[https://opskumu.oss-cn-beijing.aliyuncs.com/images/Compact.png]]
...
[[https://opskumu.oss-cn-beijing.aliyuncs.com/images/Compact-record.png]]

对于 blob，text，varchar(8099 这样的大字段，innodb 只会存放前 768 字节在数据页中，而剩余的数据则会存储在溢出段中（发生溢出情况的时候适用），对于行溢出数据，其存放采用下图中所示方法；

[[https://opskumu.oss-cn-beijing.aliyuncs.com/images/Compact-blob.png]]
...
[[https://opskumu.oss-cn-beijing.aliyuncs.com/images/Compressed-blob.png]]
...
- [[http://book.douban.com/subject/24708143/][《MySQL 技术内幕: InnoDB 存储引擎》]] 第4章 表
- [[http://venublog.com/2008/04/25/innodb-plugin-row-format-performance/][InnoDB plugin row format performance]]
- [[http://www.orczhou.com/index.php/2010/03/innodb-plugin-file-format/][InnoDB Plugin文件格式(概述)]]
- [[http://imysql.com/2014/09/28/mysql-optimization-case-blob-stored-in-innodb-optimization.shtml][优化 InnoDB 表 BLOB 列的存储效率]]
- [[http://hidba.org/?p=551][innodb 使用大字段 text，blob 的一些优化建议]]

关于 InnoDB 表 BLOB 字段的优化推荐阅读 [[http://imysql.com/2014/09/28/mysql-optimization-case-blob-stored-in-innodb-optimization.shtml][优化 InnoDB 表 BLOB 列的存储效率]] 和 [[http://hidba.org/?p=551][innodb 使用大字段 text，blob 的一些优化建议]] 这两篇文章，避免类似问题的产生。
