...
简而言之，OOM Killer 评判的值就是 =container_memory_working_set_bytes= ，可是实际对比发现，有些 Java 容器实际的内存占用和 =container_memory_working_set_bytes= 相差甚远，很多该值是 90%+ 的，实际使用工具 ps 等工具查看确只有 50% 或者更低。最后我们是通过 =container_memory_usage_bytes - container_memory_cache= 计算容器内存占用，相比 working_set 要准确多了。至于最后为什么使用这个计算，时间跨度有点长了，当时也没有记录，记得除了查资料之外还看了 docker stats 这块的源码。
...
关于应用在容器中运行要注入的一些原则，国外有人已经总结的相当好了，并且还出版了一本书 [[https://www.redhat.com/cms/managed-files/cm-oreilly-kubernetes-patterns-ebook-f19824-201910-en.pdf][《Kubernetes Patterns: Reusable Elements for Designing Cloud-Native Applications》]] 。点击书籍链接可以直接下载，红帽已经将其开源，没有版权问题。以下直接翻译摘录一部分内容：
