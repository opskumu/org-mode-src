...
#+TITLE: Docker lvm device mapper 
#+DATE: <2015-10-30 Fri>
...
direct-lvm 也是使用 LVM， device mapper the dm-thinp 内核模块，使用 direct-lvm 不再使用 loopback 设备，直接使用 raw 分区(no filesystem)，在中等负载和高密度环境下会有更好的性能优势，另外官方也是不建议生产环境系统使用默认的 LVM thin pool，至于两种类型具体的性能对比可以参考 [[https://developerblog。redhat。com/2014/09/30/overview-storage-scalability-docker/][Comprehensive Overview of Storage Scalability in Docker]] 这篇文章。
...
- ~POOL_AUTOEXTEND_THRESHOLD=60~
  - 定义自动扩容的百分比，默认为当前 pool 使用 60% 时自动扩容，100 表示 disable，最小为 50 [[http://man7.org/linux/man-pages/man7/lvmthin.7.html][lvmthin --- LVM thin provisioning]]
...
- Docker 官方文档：
  - [[https://docs.docker.com/engine/userguide/storagedriver/device-mapper-driver/][Docker and the Device Mapper storage driver]]

- 性能相关：
  - [[https://github.com/projectatomic/docker-storage-setup/issues/31#issuecomment-104387530][random I/O performance improves when increasing thinp chunksize]]
  - [[https://developerblog.redhat.com/2014/09/30/overview-storage-scalability-docker/][Comprehensive Overview of Storage Scalability in Docker]]
  - [[http://devconf.cz/files/slides2015/friday/Performance%20Tuning%20of%20Docker%20and%20RHEL%20Atomic.pdf][Performance Tuning of Docker and RHEL Atomic]]

- 部署相关：
  - [[https://access.redhat.com/articles/1492923][Managing Storage with Docker Formatted Containers on Red Hat Enterprise Linux and Red Hat Enterprise Linux Atomic Host]]
  - [[http://www.projectatomic.io/blog/2015/06/notes-on-fedora-centos-and-docker-storage-drivers/][Friends Don't Let Friends Run Docker on Loopback in Production]]
  - [[https://ekuric.wordpress.com/2015/07/17/docker-storage-setup/][docker-storage-setup]]
  - [[http://unpoucode.blogspot.hk/2015/06/docker-and-devicemappers-thinpool-in。html][Docker and devicemapper`s thinpool in RHEL 7]]
  - [[http://www.projectatomic.io/docs/docker-storage-recommendation/][Setting Up Storage]]
