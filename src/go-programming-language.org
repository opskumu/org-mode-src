...
本文为阅读 [[https://book.douban.com/subject/26337545/][The Go Programming Language]] 一书的笔记摘要。
...
*** [[https://blog.golang.org/strings][字符串]]
...
- 调用 =errors.New= 函数是非常稀少的，因为有一个方便的封装函数 =fmt.Errorf= ，它还会处理字符串格式化 - 接口只有当有两个或两个以上的 具体类型必须以相同的方式进行处理时才需要
