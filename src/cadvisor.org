...
[[https://opskumu.oss-cn-beijing.aliyuncs.com/images/cAdvisor.png]]

-----

** 入口

[[https://github.com/google/cadvisor][cAdvisor 代码托管地址]]
...
cAdvisor 首先判断自己是否运行在 container 中，如果是则 =chroot /rootfs= （cAdvisor 在 container 中运行会通过 volume 方式把系统 / mount 到 /rootfs 下）再执行 =ps -e -o user,pid,ppid,stime,pcpu,pmem,rss,vsz,stat,time,comm,cgroup= ，否则直接执行获取。(github.com/google/cadvisor/manager/container.go GetProcessList() 函数实现)
