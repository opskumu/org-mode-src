...
#+TITLE: 自定义 Kubernetes CRD
#+DATE: <2021-01-13 Wed>

** 理论概要

我们知道 Kubernetes 提供 CRD 机制用以扩展资源，在某些场景下我们可以利用 CRD 扩展通过 Kubernetes 存储资源，也就是把 Kubernetes 当作存储来使用。按照官方的说法 CRD + Controller = Operator，[[https://github.com/kubernetes-sigs/kubebuilder][kubebuilder]] 可以生成这样的代码框架，并且可自定义选择是否生成 Controller。client 相关代码则可以通过 [[https://github.com/kubernetes/code-generator][code-generator]] 生成，生成 informer、lister、clientset 等系列代码。通过这种方式，我们只需要在生成的代码中填充业务代码，而无需关心框架之外的代码，这样可以极大地提升开发效率和降低 CRD 的上手门槛。本文通过引入 ConfigMapHistory 这个 CRD 为例，带你从零到一构建和部署 CRD。

** kubebuilder 安装

kubebuilder 安装文档可以参考官方文档 [[https://book.kubebuilder.io/quick-start.html#installation][Install kubebuilder]] ：
...
完整示例代码参见 [[https://github.com/opskumu/crd-example][crd-example]]
