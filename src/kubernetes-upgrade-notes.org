...
#+BEGIN_QUOTE
Note: You cannot upgrade your cluster master more than one minor version at a time. For example, you can upgrade a cluster from version 1.6.x to 1.7.x, but not directly from 1.6.x to 1.8.x. For more information, refer to Versioning and Upgrades.  [[https://cloud.google.com/kubernetes-engine/docs/how-to/upgrading-a-cluster][Upgrading a Cluster]]
#+END_QUOTE
...
- [[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG-1.6.md#warning-etcd-backup-strongly-recommended][WARNING: etcd backup strongly recommended]]
...
- [[https://github.com/etcd-io/etcd/blob/master/Documentation/upgrades/upgrade_3_0.md#preparation][Upgrade etcd from 2.3 to 3.0]]
- [[https://coreos.com/blog/migrating-applications-etcd-v3.html][Migrating applications, clusters, and Kubernetes to etcd v3]]
...
#+BEGIN_QUOTE
[[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG-1.6.md#node-components][1.6 node components]] 
- Kubelet with the Docker-CRI implementation The Docker-CRI implementation is enabled by default. 
...
1.7 版本是针对安全、有状态服务以及可扩展功能里程碑版本，[[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG-1.7.md#major-themes][1.7.0 Major Themes]]。

根据文档 [[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG-1.7.md#action-required-before-upgrading][Action Required Before Upgrading]] 描述，如果你之前使用过 NetworkPolicy，可以参考 [[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG-1.7.md#network][Network]] 说明，其它版本选项没有大的变动。

** 1.7.x --> 1.8.x

在升级之前参考 [[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG-1.8.md#before-upgrading][1.8.0 Before Upgrading]] 官方文档，有以下几个点值得注意： 

- 默认 1.8.x 在节点启用 swap 的时候 Kubelet 会启动失败，需要额外添加选项 =--fail-swap-on=false= - ThirdPartyResource（TPR）API 被移除，需迁移至 [[https://kubernetes.io/docs/tasks/access-kubernetes-api/migrate-third-party-resource/][CustomResourceDefinition（CRD）]] - Kubelet 指定 master 节点的选项 =api-servers= 被废弃，需要通过 =--kubeconfig= 替换。
...
- [[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG-1.9.md#before-upgrading][1.9.0 Before Upgrading]]
  - =--portal-net= 、 =--service-node-ports= 选项在 =apiserver= 中被移除
  - =--network-plugin-dir= 选项在 =kubelet= 中被移除

** 1.9.x --> 1.10.x

- [[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG-1.10.md#before-upgrading][1.10.0 Before Upgrading]]
  ...
  - =apiserver= 中 =--insecure-bind-address= 、 =--insecure-port= 废弃，并会在未来版本移除，使用 =--secure-port= 、 =--bind-address= 替换 - =apiserver= 中使用 =--enable-admission-plugins= 、 =--disable-admission-plugins=  选项替换 =--admission-control=
