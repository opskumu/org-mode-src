...
- buffe [[http://wiki.answers.com/Q/Difference_between_buffer_and_cache][Difference between buffer and cache]]
...
终端执行 top 命令之后（也可后接一些选项，比如 =top -p 1=  只监控 init 进程， =top -u root= 只显示 root 运行进程等等），可以通过以下按键，实现不同功能：

- =h= ：获取 top 的命令帮助
- =1= (数字 1)：列出所有的单个 CPU 负载情况
- =z= ：top 显示颜色
  - =x= ：类似高亮显示，在 =z=  模式下使用
...
- [[http://linux.vbird.org/][鸟哥的 Linux 私房菜]]
- [[http://www.cyberciti.biz/tips/top-linux-monitoring-tools.html][top - Process Activity Command]]
- [[http://how-to.linuxcareer.com/learning-linux-commands-top][Learning Linux Commands: top]]
